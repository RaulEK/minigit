...
public class RemoveCommand implements Command {
    ...
    public void process(String[] input) throws Exception {
        if (input.length == 2) {
            List<String> filesToAdd = Files.readAllLines(Path.of(Utils.seekRepoRootFolder() + "/.minigit/addedfiles.txt"));
            String[] filesToRemove  = input[1].split(",");
            System.out.println(Arrays.toString(filesToRemove));
            for (String file :filesToRemove) {
                filesToAdd.remove(file);
            }
            if (new File(Utils.seekRepoRootFolder() + "/.minigit/addedfiles.txt").delete()) {
